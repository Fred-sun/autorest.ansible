...
class AzureRMSshPublicKey(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            public_key=dict(
                type='str',
                disposition='/properties/publicKey'
            ),
            state=dict(
                type='str',
                default='present',
    ...
    def exec_module(self, **kwargs):
        ...
        self.url = self.url.replace('{sshPublicKeyName}', self.ssh_public_key_name)

        old_response = self.get_resource()

        if not old_response:
            self.log("SshPublicKey instance doesn't exist")
            ...
            self.log('SshPublicKey instance already exists')
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log('Need to Create / Update the SshPublicKey instance')
            ...
            self.log('SshPublicKey instance deleted')
            self.results['changed'] = True
            ...
            self.log('SshPublicKey instance unchanged')
    ...
    def get_resource(self):
        ...
        try:
            ...
            # self.log("SshPublicKey instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the SshPublicKey instance.')
        if found is True:
            return response
