...
class AzureRMContainerService(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            orchestrator_profile=dict(
                type='dict',
                disposition='/properties/orchestratorProfile',
                options=dict(
                    orchestrator_type=dict(
                        type='sealed-choice',
                        disposition='/orchestratorType',
                        required=true
                    )
                )
            ),
            custom_profile=dict(
                type='dict',
                disposition='/properties/customProfile',
                options=dict(
                    orchestrator=dict(
                        type='str',
            ...
            windows_profile=dict(
                type='dict',
                disposition='/properties/windowsProfile',
                options=dict(
                    admin_username=dict(
                        type='str',
            ...
            linux_profile=dict(
                type='dict',
                disposition='/properties/linuxProfile',
                options=dict(
                    admin_username=dict(
                        type='str',
                        disposition='/adminUsername',
    ...
    def exec_module(self, **kwargs):
        ...
        self.url = self.url.replace('{containerServiceName}', self.container_service_name)

        old_response = self.get_resource()

        if not old_response:
            self.log("ContainerService instance doesn't exist")
            ...
            self.log('ContainerService instance already exists')
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log('Need to Create / Update the ContainerService instance')
            ...
            self.log('ContainerService instance deleted')
            self.results['changed'] = True
            ...
            self.log('ContainerService instance unchanged')
    ...
    def get_resource(self):
        ...
        try:
            ...
            # self.log("ContainerService instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the ContainerService instance.')
        if found is True:
            return response
